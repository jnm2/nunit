// https://raw.githubusercontent.com/dotnet/corefx/v2.0.6/src/System.Collections.Concurrent/src/System/Collections/Concurrent/BlockingCollection.cs
#if NET20 || NET35
                ...
                using (CancellationTokenSource linkedTokenSource = CancellationTokenSource.CreateLinkedTokenSource(collatedCancellationTokens))
                {
                    ...
                    if (linkedTokenSource.IsCancellationRequested)
                    {
                        if (externalCancellationToken.IsCancellationRequested) //case#3
                            ...
                            throw new ArgumentException("At least one of the specified collections is marked as complete with regards to additions.", nameof(collections));
                    }
                }
        ...
        private static int TryTakeFromAnyCoreSlow(BlockingCollection<T>[] collections, out T item, int millisecondsTimeout, bool isTakeOperation, CancellationToken externalCancellationToken)
        {
            ...
            while (millisecondsTimeout == Timeout.Infinite || timeout >= 0)
            {
                ...
                if (handles.Count == 0 && isTakeOperation) //case#5
                    throw new ArgumentException("All collections are marked as complete with regards to additions.", nameof(collections));
...
#pragma warning restore 0420
        ...
        void ICollection.CopyTo(Array array, int index)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(index), index, "The index argument must be greater than or equal zero.");
            }
            catch (ArgumentException)
            {
                throw new ArgumentException("The number of elements in the collection is greater than the available space from index to the end of the destination array.", nameof(index));
            }
            catch (RankException)
            {
                throw new ArgumentException("The array argument is multidimensional.", nameof(array));
            }
            catch (InvalidCastException)
            {
                throw new ArgumentException("The array argument is of the incorrect type.", nameof(array));
            }
        ...
        private static void ValidateCollectionsArray(BlockingCollection<T>[] collections, bool isAddOperation)
        {
            ...
            else if (collections.Length < 1)
            {
                throw new ArgumentException(
                    "The collections argument is a zero-length array.", nameof(collections));
            }
            ...
            {
                throw new ArgumentOutOfRangeException(
nameof(collections), "The collections length is greater than the supported range for 32 bit machine.");
            }
            ...
            for (int i = 0; i < collections.Length; ++i)
            {
                ...
                {
                    throw new ArgumentException(
                        "The collections argument contains at least one null element.", nameof(collections));
                }

                if (collections[i]._isDisposed)
                    throw new ObjectDisposedException(
nameof(collections), "The collections argument contains at least one disposed element.");

                if (isAddOperation && collections[i].IsAddingCompleted)
                {
                    throw new ArgumentException(
                        "At least one of the specified collections is marked as complete with regards to additions.", nameof(collections));
                }
            }
        }
        ...
        private static void ValidateTimeout(TimeSpan timeout)
        {
            ...
            if ((totalMilliseconds < 0 || totalMilliseconds > Int32.MaxValue) && (totalMilliseconds != Timeout.Infinite))
            {
                throw new ArgumentOutOfRangeException(nameof(timeout), timeout,
                    String.Format(CultureInfo.InvariantCulture, "The specified timeout must represent a value between -1 and {0}, inclusive.", Int32.MaxValue));
            }
        }
        ...
        private void CheckDisposed()
        {
            if (_isDisposed)
            {
                throw new ObjectDisposedException(nameof(BlockingCollection<T>), "The collection has been disposed.");
            }
        }
    }
